...
class TipControllerTest {
    ...
    @Nested
    @DisplayName("꿀팁 게시물 ID로 조회")
    inner class 꿀팁_게시물_ID로_조회 {
    ...
    inner class 꿀팁_좋아요_삭제 {
        ...
        inner class SuccessCase {
            ...
            fun 꿀팁_좋아요_삭제(){
                ...
                val resultActions = mock.perform(
                    MockMvcRequestBuilders
                        .delete("/api/tips/$tipId/tip-likes")
                        .header("Authorization", "Bearer $accessToken")
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON)
                )
        ...
        inner class FailCase {
            ...
            @Test
            @DisplayName("꿀팁 좋아요 삭제 : 미로그인 상태")
            fun 꿀팁_좋아요_삭제_미로그인_상태() {
                // Given
                val tipId = tips[0].id ?: fail("꿀팁을 찾을 수 없음")
            ...
            @Test
            @DisplayName("꿀팁 좋아요 삭제 : 꿀팁 ID가 존재하지 않는 경우")
